...
%{?el10:                         %global with_debuginfo 1}
%{!?with_debuginfo:              %global without_debuginfo 1}

%{?el7:%global __strip /opt/rh/devtoolset-11/root/usr/bin/strip}
%{?el8:%global __strip /opt/rh/gcc-toolset-14/root/usr/bin/strip}
%{?el9:%global __strip /opt/rh/gcc-toolset-14/root/usr/bin/strip}
%{?el8:%global __brp_strip_static_archive %{nil}}
%{?el9:%global __brp_strip_static_archive %{nil}}
...
%global compatlib             21
%global compatsrc             https://cdn.mysql.com/Downloads/MySQL-8.0/mysql-boost-%{compatver}.tar.gz
%endif
...
%endif # compatlib
%{?el8:BuildRequires:  gcc-toolset-14-annobin-annocheck}
%{?el8:BuildRequires:  gcc-toolset-14-annobin-plugin-gcc}
%{?el8:BuildRequires:  gcc-toolset-14-binutils}
%{?el8:BuildRequires:  gcc-toolset-14-dwz}
%{?el8:BuildRequires:  gcc-toolset-14-gcc-c++}
%{?el8:BuildRequires:  gcc-toolset-14-gcc}
%{?el9:BuildRequires:  gcc-toolset-14-annobin-annocheck}
%{?el9:BuildRequires:  gcc-toolset-14-annobin-plugin-gcc}
%{?el9:BuildRequires:  gcc-toolset-14-binutils}
%{?el9:BuildRequires:  gcc-toolset-14-dwz}
%{?el9:BuildRequires:  gcc-toolset-14-gcc-c++}
%{?el9:BuildRequires:  gcc-toolset-14-gcc}
%endif # el8 / el9
%if 0%{?rhel} == 10
...
%global _privatelibs libabsl_.*|libicu.*|libprotobuf.*|libmysqlharness\.so.*|libmysqlharness_stdx\.so.*|libmysqlharness_tls\.so.*|libmysqlrouter\.so\..*|libmysqlrouter_.*\.so\..*%{?ssl_bundled:|libssl\.so\..*|libcrypto\.so\..*}|libfido2.*|libpolyglot.*|libjitexecutor.*
...
The MySQL web site (https://www.mysql.com/) provides the latest
...
The MySQL web site (https://www.mysql.com/) provides the latest news and
...
Obsoletes:      mariadb
Obsoletes:      mariadb-client-utils
...
Obsoletes:      mysql8.4-libs < %{version}-%{release}
...
The MySQL web site (https://www.mysql.com/) provides the latest news
...
%endif # 0%{?compatlib} && 0%{?rhel} == 7

%if 0%{?compatlib} && 0%{?rhel} > 7
(
    pushd mysql-%{compatver}/build/libmysql
    make DESTDIR=%{buildroot} mysqlclient install
    rm -f %{buildroot}%{_libdir}/mysql/libmysqlclient.{a,so}
    popd
)
%endif # 0%{?compatlib} && 0%{?rhel} > 7
...
%attr(755, root, root) %{_libdir}/mysql/private/libicui18n.so.77.1
%attr(755, root, root) %{_libdir}/mysql/private/libicustubdata.so.77.1
%attr(755, root, root) %{_libdir}/mysql/private/libicuuc.so.77.1
...
%attr(755, root, root) %{_libdir}/mysql/plugin/component_audit_log.so
%attr(644, root, root) %{_datadir}/mysql-*/audit_log_component_filter_install.sql
%attr(644, root, root) %{_datadir}/mysql-*/audit_log_component_filter_uninstall.sql
...
%attr(755, root, root) %{_libdir}/mysql/plugin/component_firewall.so
%attr(755, root, root) %{_libdir}/mysql/plugin/component_object_policy.so
...
%attr(644, root, root) %{_datadir}/mysql-*/install_component_firewall.sql
%attr(644, root, root) %{_datadir}/mysql-*/uninstall_component_firewall.sql
%attr(644, root, root) %{_datadir}/mysql-*/firewall_component_to_plugin.sql
%attr(644, root, root) %{_datadir}/mysql-*/firewall_plugin_to_component.sql
%attr(644, root, root) %{_datadir}/mysql-*/install_component_object_policy.sql
%attr(644, root, root) %{_datadir}/mysql-*/uninstall_component_object_policy.sql
%if 0%{?aws_sdk}
...
%attr(755, root, root) %{_libdir}/mysql/plugin/debug/component_audit_log.so
...
%attr(755, root, root) %{_libdir}/mysql/plugin/debug/component_firewall.so
%attr(755, root, root) %{_libdir}/mysql/plugin/debug/component_object_policy.so
%if 0%{?aws_sdk}
...
%{_libdir}/mysql/private/icudt77l/cnvalias.icu
%{_libdir}/mysql/private/icudt77l/uemoji.icu
%{_libdir}/mysql/private/icudt77l/ulayout.icu
%{_libdir}/mysql/private/icudt77l/unames.icu
...
%if 0%{?commercial}
%attr(755, root, root) %{_bindir}/mysqldm
%endif # commercial
...
%{_libdir}/mysqlrouter/private/libicui18n.so.77.1
%{_libdir}/mysqlrouter/private/libicustubdata.so.77.1
%{_libdir}/mysqlrouter/private/libicuuc.so.77.1
...
%changelog
* Fri Feb  6 2026 Harin Vadodaria <harin.vadodaria@oracle.com> - 9.7.0
- Added component_object_policy

* Mon Feb  2 2026 Marc Alff <marc.alff@oracle.com> - 9.7.0
- Moved component_telemetry to community

* Wed Jan 28 2026 Marc Alff <marc.alff@oracle.com> - 9.7.0
- Moved telemetry_client to community
- Moved component_test_telemetry_resource_provider to community
- Moved component_test_telemetry_secret_provider to community

* Thu Oct  9 2025 Marek Szymczak <marek.szymczak@oracle.com> - 9.6.0-1
- Added component_audit_log
- Added component_test_mysql_file_service

* Fri Aug  1 2025 Marc Alff <marc.alff@oracle.com> - 9.5.0-1
- Added component_test_telemetry_resource_provider
- Added component_test_telemetry_secret_provider

* Fri Jun 27 2025 Murthy Sidagam <venkata.sidagam@oracle.com> - 9.5.0-1
- Added component_keyring_kmip.so component

* Wed Jun 11 2025 Krzysztof Horszczaruk <krzysztof.horszczaruk@oracle.com> - 9.5.0-1
- Added component_test_component_init_fail

* Tue May 27 2025 Michal Jankowski <michal.jankowski@oracle.com> - 9.4.0-1
- Added component_firewall.so

* Wed Apr 30 2025 Krzysztof Horszczaruk <krzysztof.horszczaruk@oracle.com> - 9.4.0-1
- Added component_test_component_deinit_no_deadlock

* Wed Apr 30 2025 Krzysztof Horszczaruk <krzysztof.horszczaruk@oracle.com> - 9.4.0-1
- Added component_test_component_init_then_register
...
* Wed Nov 13 2024 Samar Pratap Singh <samar.pratap.singh@oracle.com> - 9.2.0-1
- Added component_connection_control

* Wed Jul 10 2024 Samar Pratap Singh <samar.pratap.singh@oracle.com> - 9.1.0-1
- Added authentication_openid_connect plugin
- Added mysql_test_jwt_generator tool

* Wed May 29 2024 Miroslav Rajcic <miroslav.rajcic@oracle.com> - 9.1.0-1
- Added component_test_server_telemetry_logs_* test components
- Added component_keyring_aws

* Mon May 27 2024 Georgi "Joro" Kodinov <georgi.kodinov@oracle.com> - 9.0.0-1
- Added the option usage tracker component

* Mon May 20 2024 Murthy Sidagam <venkata.sidagam@oracle.com> - 9.0
- Added component_test_session_var_service.so component

* Mon Feb 12 2024 Samar Pratap Singh <samar.pratap.singh@oracle.com> - 8.4.0-1
- Removed keyring_file plugin

* Tue Jan 30 2024 Samar Pratap Singh <samar.pratap.singh@oracle.com> - 8.4.0-1
- Removed keyring_encrypted_file plugin

* Thu Jan 25 2024 Georgi "Joro" Kodinov <Georgi.Kodinov@oracle.com> - 8.4.0-1
- Removed mysql_ssl_rsa_setup

* Tue Jan 09 2024 Harin Vadodaria <harin.vadodaria@oracle.com> - 8.4.0-1
- Removed openssl_udf plugin

* Tue Jan 02 2024 Harin Vadodaria <harin.vadodaria@oracle.com> - 8.4.0-1
- Removed references to WITH_AUTHENTICATION_FIDO

* Mon Nov 20 2023 Samar Pratap Singh <samar.pratap.singh@oracle.com> - 8.4.0-1
- Removed keyring_oci plugin

* Fri Nov 17 2023 Miroslav Rajcic <miroslav.rajcic@oracle.com> - 8.4.0-1
